...
class RegExpSyntaxError extends SyntaxError {
    constructor(source, flags, index, message) {
        if (source) {
            if (!source.startsWith("/")) {
                source = `/${source}/${flags.unicode ? "u" : ""}${flags.unicodeSets ? "v" : ""}`;
            }
            source = `: ${source}`;
        }
...
class RegExpValidator {
    ...
    _parseFlagsOptionToMode(uFlagOrFlags, source, sourceEnd) {
        let unicode = false;
        let unicodeSets = false;
        ...
        if (unicode && unicodeSets) {
            throw new RegExpSyntaxError(source, { unicode, unicodeSets }, sourceEnd + 1, "Invalid regular expression flags");
        }
    ...
    raise(message) {
        throw new RegExpSyntaxError(this.source, {
